...
class TokenGenerator(object):
    ...
    def generate(self, user, data=None):
        ...
        """
        data = data or {}
        data.update({u'uid': self._uid(user), })
        return signing.dumps(data, salt=__name__).replace(u':', u'.')

    def is_valid(self, user, signed_value):
        try:
            self.data = signing.loads(signed_value.replace(u'.', u':'), salt=__name__)
        except signing.BadSignature:
            return False

        if self.data[u'uid'] != self._uid(user):
            return False
...
class UserActivationTokenGenerator(TokenGenerator):
    ...
    def _uid(self, user):
        return u';'.join((smart_text(user.pk), smart_text(user.is_active)))
...
class UserEmailChangeTokenGenerator(TokenGenerator):
    ...
    def generate(self, user, new_email):
        return super(UserEmailChangeTokenGenerator, self).generate(user, {u'new_email': new_email, })

    def get_email(self):
        return self.data[u'new_email']
